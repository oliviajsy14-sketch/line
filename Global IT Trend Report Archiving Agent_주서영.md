...
- [Machine-readable Query Table 예시](#machine-readable-query-table-예시)
- [전체 작업 순서](#전체-작업-순서)
- [Check Box 운영 기준](#check-box-운영-기준)
- [n/a 입력 기준](#na-입력-기준)
- [n/a와 미검색 상태 구분](#na와-미검색-상태-구분)
...
- [Google Query 날짜 범위 적용 기준](#google-query-날짜-범위-적용-기준)
- [Google Search 검색어 작성 방식](#google-search-검색어-작성-방식)
- [Google News 날짜 필터 기준](#google-news-날짜-필터-기준)
- [기사 날짜 판정 우선순위](#기사-날짜-판정-우선순위)
- [날짜 기준 최종 포함 여부](#날짜-기준-최종-포함-여부)
- [기사 선별 기준](#기사-선별-기준)
...
- [public beta / 내부 테스트 큰따옴표 구분](#public-beta-내부-테스트-큰따옴표-구분)
- [출시 예정 기능 큰따옴표 기준](#출시-예정-기능-큰따옴표-기준)
- [큰따옴표 적용 예시](#큰따옴표-적용-예시)
- [Query Source Mapping 예시](#query-source-mapping-예시)
...
- Google Query 또는 Google News/Search 검색 시에도 작업자가 입력한 `date_range`를 동일하게 적용한다.
- `date_range`는 시작일과 종료일을 모두 포함하는 inclusive range로 처리한다.
- 종료일 다음날까지 임의로 확장하지 않는다.
...
8. 동일 이벤트 여부를 내부 중복 판단 기준으로 확인
9. 대표 URL과 보완 가치가 있는 보조 URL 판단
...
## Machine-readable Query Table 예시

이 표는 자동화용 구조 예시이며, 실제 실행 시에는 위의 Weekly Sheet Query / Global IT Trend Sheet Query 전체 리스트를 기준으로 한다.
...
- 이 표는 구조 예시이며, 전체 Query List를 대체하지 않는다.
- 실제 자동화 실행 시에는 `Query 전체 리스트`의 Weekly Sheet Query와 Global IT Trend Sheet Query를 기준으로 한다.
- 추후 자동화 코드화 단계에서 전체 Query List를 CSV, YAML, JSON 등 별도 파일로 분리할 수 있다.
...
| 날짜 필터 적용 불가 | 검색 결과의 원문 발행일을 직접 확인해 date_range 포함 여부 판단 |
| 검색 결과 날짜와 원문 날짜 불일치 | 원문 날짜를 우선 기준으로 적용 |
| 날짜 확인 불가 | Sheet 입력 금지, 실행 로그에 기록 |
| 종료일 당일 기사 | date_range에 포함되는 기사로 판단 |
...
- Google Query 또는 Google News/Search를 사용할 때는 작업자가 입력한 `date_range`를 기준으로 날짜 범위를 반드시 적용
...
## Google Query 날짜 범위 적용 기준

- Google Query 또는 Google News/Search 검색 시 작업자가 입력한 `date_range` 범위 안에서만 기사를 검색한다.
- `date_range`는 시작일과 종료일을 모두 포함하는 기간으로 처리한다.
- 예: `2026.6.7~2026.6.24` 입력 시 검색 대상은 `2026.6.7`부터 `2026.6.24`까지 발행된 기사다.
- 종료일 다음날까지 확장하지 않는다.
- 검색 연산자 또는 날짜 필터를 사용할 수 있는 경우 입력된 시작일과 종료일을 그대로 적용한다.
- 예: `2026.6.7~2026.6.24` 검색 시 날짜 조건은 `2026-06-07`부터 `2026-06-24`까지로 적용한다.
- 검색 결과가 입력된 시작일보다 이전이거나 종료일보다 이후이면 Sheet에 입력하지 않는다.
- 기사 발행일, 업데이트일, 본문 내 날짜를 확인해 최종 날짜 범위 포함 여부를 판단한다.
- 날짜가 불명확한 기사는 Sheet에 바로 입력하지 않고 실행 로그에 기록한다.

## Google Search 검색어 작성 방식

Google Search 또는 Google News 검색 시 아래 방식으로 검색어를 구성한다.

- 기본 검색어: `Query명 + 핵심 키워드`
- 날짜 조건: 작업자가 입력한 `date_range` 기준 적용
- 검색 대상 기간은 시작일 이상, 종료일 이하로 판단한다.
- 검색 연산자를 사용할 수 있는 환경에서는 날짜 조건을 함께 넣는다.
- 예: `OpenAI AI Agent 2026-06-07 2026-06-24`
- 예: `OpenAI AI Agent after:2026-06-07 before:2026-06-24`
- 단, 어떤 검색 연산자를 사용하더라도 최종 포함 여부는 기사 발행일이 `date_range` 안에 있는지로 판단한다.
- 검색 연산자 결과와 실제 기사 발행일이 다르면 실제 기사 발행일을 우선한다.

## Google News 날짜 필터 기준

- Google News에서 날짜 필터를 사용할 수 있으면 `date_range`와 동일한 시작일과 종료일을 설정한다.
- 예: `2026.6.7~2026.6.24` 입력 시 Google News 필터도 `2026.6.7~2026.6.24`로 설정한다.
- Google News에서 날짜 필터를 직접 설정할 수 없는 경우 검색 결과의 발행일을 직접 확인한다.
- 발행일이 시작일 이상, 종료일 이하인 기사만 Sheet 입력 대상으로 판단한다.
...
1. 기사 원문 발행일
2. 기사 원문 업데이트일
3. 공식 Blog / Newsroom / Release Notes의 게시일
4. Google News 또는 Search 결과에 표시된 날짜
5. 본문 내 발표일 또는 공개일

운영 기준:

- 원문 발행일이 확인되면 원문 발행일을 우선한다.
- 업데이트일만 있는 경우 업데이트일을 기준으로 판단한다.
- 검색 결과 날짜와 원문 날짜가 다르면 원문 날짜를 우선한다.
- 날짜가 확인되지 않으면 Sheet에 입력하지 않고 실행 로그에 기록한다.

## 날짜 기준 최종 포함 여부

- 기사 날짜가 `date_range` 시작일보다 이전이면 제외한다.
- 기사 날짜가 `date_range` 종료일보다 이후이면 제외한다.
- 기사 날짜가 시작일과 같은 경우 포함한다.
- 기사 날짜가 종료일과 같은 경우 포함한다.
- 즉, `date_range`는 시작일과 종료일을 모두 포함하는 inclusive range로 처리한다.
- 예: `2026.6.7~2026.6.24`이면 `2026.6.7` 기사와 `2026.6.24` 기사 모두 포함한다.
...
- AI/GPT에 넣기 애매한 경우 내부 판단 기준으로 `AI 관련성 약함`, `카테고리 재검토`, `Global IT 후보` 등을 활용
...
- 최종적으로 각 Query 안에서는 `시장 영향이 큰 기사 → 기업/서비스 변화 기사 → 일반 관련 기사 → 보완 URL 검토 대상` 순서로 정렬
- 같은 내용의 중복 기사는 Sheet에 별도 행으로 반복 입력하지 않는다. 보완 가치가 있는 출처는 대표 기사 URL 셀에 줄바꿈으로 함께 입력하고, 정보 가치가 낮은 중복 기사는 실행 로그에만 기록한다.
...
- Naver/LINE 단독 기사
- 동일 내용이 이미 대표 기사로 정리되어 있고, 보완 URL로도 가치가 없는 완전 중복 기사
...
6. 신규 정보가 없으면 Sheet에 별도 행으로 반복 입력하지 않고, 필요 시 실행 로그에 기록

### 내부 판단 기준 표기

- 신규 정보 있음: `후속 기사 유지`
- 신규 정보 없음: Sheet 추가 입력 제외 또는 실행 로그 기록
- 대표 기사만 유지: `대표 기사 유지`
- 판단 애매함: `중복 가능성`
...
- 중복 제거, 내부 중복 판단, 제외 판단, AI 관련성 판단은 업무 프로세스 기준으로 수행하되 Sheet 컬럼으로 입력하지 않음
...
- [ ] 검색 기간이 `yyyy.m.d~yyyy.m.d` 형식인지 확인
- [ ] Google Query 또는 Google News/Search 검색 시 입력된 `date_range`가 적용되었는지 확인
- [ ] `date_range`가 시작일과 종료일을 모두 포함하는 inclusive range로 처리되었는지 확인
- [ ] 종료일 다음날로 임의 확장하지 않았는지 확인
- [ ] 기사 발행일이 시작일 이상, 종료일 이하인지 확인
- [ ] 날짜가 불명확한 기사가 Sheet에 입력되지 않고 실행 로그에 기록되었는지 확인
...
- [ ] 각 Query 섹션 안에서 `시장 영향이 큰 기사 → 기업/서비스 변화 기사 → 일반 관련 기사 → 보완 URL 검토 대상` 순서로 정렬했는지 확인
...
- [ ] 대분류, Query / Service, Korean Title, Check Box, URL 구조로 Sheet가 정리되었는지 확인
...
- [ ] 정식 릴리스, 정식 출시, public beta, 공식 출시 예정인 신규 기능명·서비스명에만 큰따옴표를 사용했는지 확인
...
- [ ] 목차 순서와 실제 본문 순서가 일치하는지 확인
- [ ] 중복 기사 관련 표현이 URL 셀 줄바꿈 또는 실행 로그 기준으로 통일되었는지 확인
- [ ] Korean Title 큰따옴표 룰이 중복 없이 정리되었는지 확인
- [ ] 예시 Korean Title이 큰따옴표 기준과 충돌하지 않는지 확인
- [ ] Machine-readable Query Table이 예시용임을 명확히 표시했는지 확인
- [ ] 날짜 형식이 `yyyy.m.d` 기준으로 전체 통일되었는지 확인
...
- 목차 순서와 본문 섹션 순서 정합성 정리
- 중복 기사 관련 잔여 표현을 URL 셀 줄바꿈 또는 실행 로그 기준으로 통일
- Cluster 표현을 내부 중복 판단 기준으로 제한
- Korean Title 큰따옴표 룰 중복 문장 정리
- 예시 Korean Title과 큰따옴표 적용 기준 정합성 수정
- Machine-readable Query Table을 예시용 섹션으로 명확화
- 날짜 형식 잔여 표현 재검수
- 최종 산출물 기준에서 금지 컬럼 관련 표현 재정리
- Google Query 및 Google News/Search 검색 시 date_range 적용 기준 추가
- date_range를 시작일과 종료일 모두 포함하는 inclusive range로 정의
- 종료일 다음날로 임의 확장하지 않는 기준 추가
- 검색 결과 날짜보다 원문 발행일을 우선하는 날짜 판정 기준 추가
- 날짜 확인 불가 기사는 Sheet가 아니라 실행 로그에 기록하는 기준 추가
